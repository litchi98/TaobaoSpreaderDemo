...
public class ScrollingUtil {
    ...
    public static boolean isViewToTop(View view, int mTouchSlop) {
        ...
        if (view instanceof CusNestedScrollView) return isCusNestedScrollViewToTop((CusNestedScrollView)view);
        return (view != null && Math.abs(view.getScrollY()) <= 2 * mTouchSlop);
    }

    private static boolean isCusNestedScrollViewToTop(CusNestedScrollView view) {
        return view.getVerticalScrollDistance() == 0;
    }
    ...
    public static boolean isViewToBottom(View view, int mTouchSlop) {
        ...
        if (view instanceof CusNestedScrollView) return isCusNestedScrollViewToBottom((CusNestedScrollView) view);
        if (view instanceof ViewGroup) return isViewGroupToBottom((ViewGroup) view);
        return false;
    }

    private static boolean isCusNestedScrollViewToBottom(CusNestedScrollView view) {
